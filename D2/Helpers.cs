...
namespace D2R_MULTILAUNCHER
{
    class Helpers
    {
        ...
        public static List<AccountInfo> getAccountProfiles()
        {
            List<AccountInfo> _accounts = new List<AccountInfo>();
            try
            {
                ...
                foreach (var file in account_directory_info.EnumerateFiles("*.encrypted.json"))
                {
                    ...
                    {
                        AccountInfo accountObj = new AccountInfo();
                        accountObj.ReadFromBinaryFile(file.FullName);
                        _accounts.Add(accountObj);
                    }
                    catch { }
                }
        ...
        public static void KillD2RInstanceCheckerHandle(Process gameProcess)
        {
            ...
            Debug.WriteLine("KillD2RInstanceChecker -> Start.");
            ...
            try
            {
                ...
                {
                    Debug.WriteLine("KillD2RInstanceChecker -> Process not found.");
                    return;
                }
                        ...
                        using (dupHandle)
                        {
                            ...
                            if (!String.IsNullOrEmpty(objectName))
                            {
                                if (objectName.Contains("DiabloII Check For Other Instances"))
                                {
                                    ...
                                    Debug.WriteLine("Closing Handle: " + System.String.Format("0x{0:X}", checkford2launcherhandle));
        ...
        public static void KillAllD2RInstanceCheckerHandle()
        {
            ...
            //    }
            //}
            string handlePath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Application.ExecutablePath), "handle.exe");
            //判断文件是否存在
            if (System.IO.File.Exists(handlePath))
            {
                //执行handle.exe文件
                string pth = handlePath;
                string arguments = "-a -p D2R.exe Instances  -nobanner";
                Process process = new Process();
                process.StartInfo.FileName = "cmd.exe";
                process.StartInfo.Arguments = $"/C \"{pth}\" {arguments}";
                ...
                foreach (string line in lines)
                {
                    ...
                    if (parts.Length > 1)
                    {
                        string sstxtA = parts[1].Trim();
                        string sstxtB = parts[2].Trim();
                        int x = sstxtA.IndexOf(' ') - 1;
                        string D2rPID = sstxtA.Substring(0, x + 1); // 更新此行
                        string[] sstxtBparts = sstxtB.Split(new[] { " " }, StringSplitOptions.RemoveEmptyEntries);
                        string handle = sstxtBparts[1];
                        string cmdt = $"handle -c {handle} -p {D2rPID} -y";
                        Process killProcess = new Process();
                        killProcess.StartInfo.FileName = "cmd.exe";
                        killProcess.StartInfo.Arguments = $"/C {cmdt}";
                        killProcess.StartInfo.Verb = "runas";
                        killProcess.StartInfo.CreateNoWindow = true; // 不创建新窗口
                        killProcess.Start();
                        // 等待进程结束
            ...
            {
                MessageBox.Show("handle.exe文件不存在！");
            }
        }
        public static int KillAllD2RInstanceCheckerHandle_return()
        {
            string handlePath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Application.ExecutablePath), "handle.exe");
            //判断文件是否存在
            if (System.IO.File.Exists(handlePath))
            {
                ...
                foreach (string line in lines)
                {
                    ...
                    if (parts.Length > 1)
                    {
                        ...
                        //MessageBoxEx.Show("未发现Check For Other Instances进程");
                        //未发现Check For Other Instances进程
                        return -1;
                    }
                }
            }
